...
TEST_CASE("testing $set operator with dotpath")
{
  ...
  SUBCASE("updateOne $set nested.enc2")
  {
    ...
    const auto updateCommand = jsonFromFile(
      "../test_v3/data/find_predicates/set_operator_dotpath/set_operator_dotpath_update.json"
                                           );
  ...
  SUBCASE("updateMany $set nested.plain2 (non-encrypted)")
  {
    ...
    const auto updatedCommand = jsonFromFile(
      "../test_v3/data/find_predicates/set_operator_dotpath/set_operator_dotpath_update_plain.json"
      );
    collectionServicePtr->updateManyFromJson(updatedCommand["updates"][0]["q"], updatedCommand["updates"][0]["u"]);
