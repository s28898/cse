...
TEST_CASE("testing $eq operator")
{
  ...
  const auto docsToInsert = jsonFromFile("../test_v3/data/find_predicates/eq_operator/eq_operator_insert.json");
  collectionServicePtr->insertManyFromJson(docsToInsert);

  const auto findQuery = jsonFromFile("../test_v3/data/find_predicates/eq_operator/eq_operator_find.json");
