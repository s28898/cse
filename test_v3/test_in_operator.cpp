...
TEST_CASE("testing $in operator")
{
  ...
  const auto docsToInsert =
    jsonFromFile("../test_v3/data/find_predicates/in_operator/in_operator_insert.json");
  collectionServicePtr->insertManyFromJson(docsToInsert);

  const auto findQuery = jsonFromFile("../test_v3/data/find_predicates/in_operator/in_operator_find.json");
