...
struct BsonDecryptorV3
{
    ...
    EncKind encryption_kind_for_path(std::size_t idx) const
    {
      ...
      // Legacy fallback for older validators/tests:
      if (s == "AES")
        return EncKind::AES256_CTR;
    ...
    // ------------------------------------------------------------
    // Primitive decoding helpers (PoC-friendly)
    // ------------------------------------------------------------
...
//    }
    ...
    static bool is_valid_utf8(std::string_view s)
    {
      ...
      while (p < end)
      {
        ...
        int need = 0;
        if ((c >> 5) == 0x6) need = 1;         // 110xxxxx
        else if ((c >> 4) == 0xE) need = 2;    // 1110xxxx
        else if ((c >> 3) == 0x1E) need = 3;   // 11110xxx
        else return false;
      ...
      for (const auto& el : object)
      {
        ...
        if (path_matches())
        {
          ...
          switch (t)
          {
            ...
            default:
              // PoC fallback: treat unknown as string
              out.append(kvp(key, decrypt_string_from_aes(bin, enc)));
              break;
          }
