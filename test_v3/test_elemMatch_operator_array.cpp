...
TEST_CASE("testing $elemMatch operator on arrays")
{
  ...
  const auto docsToInsert =
    jsonFromFile("../test_v3/data/find_predicates/elemMatch_operator_array/elemMatch_operator_array_insert.json");
  collectionServicePtr->insertManyFromJson(docsToInsert);
  
  const auto findQuery = jsonFromFile(
    "../test_v3/data/find_predicates/elemMatch_operator_array/elemMatch_operator_array_find.json"
                                     );
