...
TEST_CASE("testing $not operator")
{
  ...
  const auto docsToInsert =
    jsonFromFile("../test_v3/data/find_predicates/not_operator/not_operator_insert.json");
  collectionServicePtr->insertManyFromJson(docsToInsert);

  const auto findQuery = jsonFromFile("../test_v3/data/find_predicates/not_operator/not_operator_find.json");
