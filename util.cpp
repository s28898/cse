...
/*
 * OBECNIE ODSZYFROWUJĘ k_binary DO b_string
 * OSTATECZNIE POWINIENEM PAMIĘTAĆ, JAKI TYP BYŁ UŻYTY ORYGINALNIE
 */
...
//  for (const bsoncxx::document::element &element: document)
//  {
//    if (element.type() == bsoncxx::type::k_binary)
//    {
////      throw std::runtime_error("k_binary!");
//      auto decrypted = encryption_agent.decrypt_AES(element.get_binary());
//      print_binstr(decrypted);
//      auto as_int = bytes_to_i64(decrypted);
//      auto* leak = new auto(decrypted);
//      result.append(
//        kvp(
//          element.key(),
//          bsoncxx::types::b_string{      std::to_string(as_int)}
//           )
//                   );
//    }
//    else
