...
TEST_CASE("testing $lt and $lte")
{
  ...
  {
   const auto findQuery = jsonFromFile("../test_v3/data/range_predicates/range_predicates_find_lt.json");
  ...
  {
    const auto findQuery = jsonFromFile("../test_v3/data/range_predicates/range_predicates_find_lte.json");
    auto cursor = collectionServicePtr->findManyFromJson(findQuery);
    const auto result = jsonArrayFromCursor(cursor);
