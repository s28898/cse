...
#ifndef MONGO_2_SCHEMAVALIDATORTRANSFORMER_V3_H
#define MONGO_2_SCHEMAVALIDATORTRANSFORMER_V3_H
...
struct SchemaValidatorTransformerV3
{
    public:
    ...
    static void require(const ordered_json& document, std::string_view key)
    {
      if (!document.contains(key))
        throw std::runtime_error(std::string("SchemaValidatorTransformerV3: missing key: ") + std::string(key));
    }
    ...
    {
      if (trunk.empty())
        throw std::runtime_error("SchemaValidatorTransformerV3: pop_trunk on empty stack");
      trunk.pop_back();
    }
...
#endif // MONGO_2_SCHEMAVALIDATORTRANSFORMER_V3_H